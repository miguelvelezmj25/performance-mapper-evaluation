...
public class BenchCThread {
  ...
  private void processNewOrder() throws SQLException {
    ...
    for (int number = 1; number <= olCnt; number++) {
      ...
      if (data.contains("original") && sData.contains("original")) {
        bg[number - 1] = 'B';
      } else {
        bg[number - 1] = 'G';
      }
      ...
      BigDecimal olAmount =
          new BigDecimal(olQuantity)
              .multiply(price)
              .multiply(ONE.add(wTax).add(tax))
  ...
  private void processPayment() throws SQLException {
    ...
    if (byName) {
      prep =
          prepare("SELECT COUNT(C_ID) FROM CUSTOMER " + "WHERE C_LAST=? AND C_D_ID=? AND C_W_ID=?");
    ...
    if (credit.equals("BC")) {
      prep =
          prepare("SELECT C_DATA INTO FROM CUSTOMER " + "WHERE C_ID=? AND C_D_ID=? AND C_W_ID=?");
      prep.setInt(1, cId);
      prep.setInt(2, cdId);
      prep.setInt(3, wId);
      rs = db.query(prep);
  ...
  private void processDelivery() throws SQLException {
    ...
    for (int dId = 1; dId <= bench.districtsPerWarehouse; dId++) {
      ...
      if (noId != -1) {
        ...
        rs.close();
        prep =
            prepare("UPDATE ORDERS SET O_CARRIER_ID=? " + "WHERE O_ID=? AND O_D_ID=? AND O_W_ID=?");
