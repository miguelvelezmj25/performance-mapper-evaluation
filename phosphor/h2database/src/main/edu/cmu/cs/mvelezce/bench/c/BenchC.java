package edu.cmu.cs.mvelezce.bench.c;

import edu.cmu.cs.mvelezce.bench.Bench;
import edu.cmu.cs.mvelezce.bench.Database;
...
/**
 * This test is similar to the TPC-C test of the Transaction Processing Council (TPC). Only one
 * connection and one thread is used. Referential integrity is not implemented.
 *
 * <p>See also http://www.tpc.org
 */
public class BenchC implements Bench {
  ...
  private void loadCustomerSub(int dId, int wId) throws SQLException {
    ...
    PreparedStatement prepCustomer =
        database.prepare(
  ...
  private void loadOrderSub(int dId, int wId) throws SQLException {
    ...
    PreparedStatement prepOrder =
        database.prepare(
            "INSERT INTO ORDERS(O_ID, O_C_ID, O_D_ID, O_W_ID, "
                + "O_ENTRY_D, O_CARRIER_ID, O_OL_CNT, O_ALL_LOCAL) "
                + "VALUES(?, ?, ?, ?, ?, ?, ?, 1)");
    PreparedStatement prepNewOrder =
        database.prepare("INSERT INTO NEW_ORDER (NO_O_ID, NO_D_ID, NO_W_ID) " + "VALUES (?, ?, ?)");
    PreparedStatement prepLine =
        database.prepare(
