...
/**
 * Measures performance of a full scan with DiskOrderedCursor compared to a regular cursor walk.
 *
 * <p>The DB population and scan are run as two separate invocations, so that the file system cache
 * can be cleared prior to the scan.
 *
 * <p>To populate the DB we insert and update (with the same value) each record to produce a log
 * that is around 50% utilized, to add realism. With insert-only, a log that is 100% utilized would
 * result in less real/random IO.
 *
 * <p>Variants on scan: - key-only or key-plus-data - with or without duplicates - preload or not
 *
 * <p>Other scan parameters: - internalMemoryLimit - lsnBatchSize
 *
 * <p>Data set parameters, normally held constant across multiple runs: - writes are in key order or
 * random order - number of records - key and data size - dups per key (for dup db) - JE cache size
 *
 * <p>MeasureDiskOrderedScan.sh (in this directory) is a script for running this program that can be
 * copied and modified to run a subset of tests.
 */
public class ScanLoops {
  ...
  private ScanLoops.Action action = Action.DiskOrderedScan;
