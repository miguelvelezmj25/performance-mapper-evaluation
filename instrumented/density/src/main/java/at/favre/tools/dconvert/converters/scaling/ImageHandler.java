...
public class ImageHandler {
    ...
    public List<File> saveToFile(File targetFile, LoadedImage imageData, Dimension targetDimension, boolean isNinePatch) throws Exception {
        ...
        if(args.outputCompressionMode != null) {
            for(ImageType.ECompression compression : compressionList) {
                ...
                if(args.upScalingAlgorithm != null || args.downScalingAlgorithm != null) {
...
//    }
    ...
    private List<ScaleAlgorithm> getScaleAlgorithm(EScalingAlgorithm algorithm, EScalingAlgorithm.Type type) {
        if(TEST_MODE) {
            Predicate<EScalingAlgorithm> predicate = new MyPredicate<>(type);
            Function<EScalingAlgorithm, ScaleAlgorithm> function = new MyFunction();
            return EScalingAlgorithm.getAllEnabled().stream().filter(predicate).map(function).collect(Collectors.toList());

//            return EScalingAlgorithm.getAllEnabled().stream().filter(new Predicate<EScalingAlgorithm>() {
//                @Override
//                public boolean test(EScalingAlgorithm eScalingAlgorithm) {
//                    return eScalingAlgorithm.getSupportedForType().contains(type);
//                }
//            }).map(new Function<EScalingAlgorithm, ScaleAlgorithm>() {
//                @Override
...
//            }).collect(Collectors.toList());
    ...
    public BufferedImage scale(ScaleAlgorithm scaleAlgorithm, BufferedImage imageToScale, int dWidth, int dHeight, ImageType.ECompression compression, Color background) {
...
//    }
    ...
    private ScaleAlgorithm getAsScalingAlgorithm(final ScaleAlgorithm algorithm, ImageType.ECompression compression) {
        return new MyScalingAlgorithm(this, algorithm, compression, DEFAULT_COLOR);
    }
}
