...
public class Turn {
  ...
  public synchronized Turn next() {
    if (_allowed < 0) {
      throw new IllegalStateException(
          "All transaction processing is now aborted, probably due to an earlier IOException.");
    }
    if (_next == null) {
  ...
  /**
   * Abort the pipeline. Prevents this or any following turn from continuing, but doesn't affect
   * preceding turns already further along in the pipeline.
   *
