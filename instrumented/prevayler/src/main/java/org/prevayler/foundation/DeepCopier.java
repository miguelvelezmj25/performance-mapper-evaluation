...
public class DeepCopier {
   ...
   * memory. Recommended for relatively small objects, such as individual transactions.
   */
  ...
  /**
   * Produce a deep copy of the given object. Serializes the object through a pipe between two
   * threads. Recommended for very large objects, such as an entire prevalent system. The current
   * thread is used for serializing the original object in order to respect any synchronization the
   * caller may have around it, and a new thread is used for deserializing the copy.
   */
  ...
  private static class Receiver extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        // The object has been successfully deserialized, so ignore problems at this point (for
        // example,
        // the serializer may have explicitly closed the _inputStream itself, causing this read to
        // fail).
      }
    }
