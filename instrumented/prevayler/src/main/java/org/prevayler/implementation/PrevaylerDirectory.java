...
public class PrevaylerDirectory {
  ...
  private static final String SNAPSHOT_FILENAME_PATTERN =
      "\\d{" + DIGITS_IN_FILENAME + "}\\." + SNAPSHOT_SUFFIX_PATTERN;
  private static final String JOURNAL_SUFFIX_PATTERN = "[a-zA-Z0-9]*[Jj]ournal";
  private static final String JOURNAL_FILENAME_PATTERN =
      "\\d{" + DIGITS_IN_FILENAME + "}\\." + JOURNAL_SUFFIX_PATTERN;
   ...
   *
   * <p>Snapshot suffixes must match the pattern /[a-zA-Z0-9]*[Ss]napshot/.
   *
   ...
   *
   * <p>Journal suffixes must match the pattern /[a-zA-Z0-9]*[Jj]ournal/.
   *
   * @throws IllegalArgumentException otherwise.
   */
   ...
   *
   * <p>Returns -1 if file does not have a valid snapshot filename.
   */
   ...
   *
   * <p>Returns -1 if file does not have a valid journal filename.
   */
  ...
  private File file(long version, String suffix) {
    if (version < 0) {
      throw new IllegalArgumentException(
          "Snapshot and journal version numbers must be non-negative: " + version);
    }
    String fileName = "0000000000000000000" + version;
    return new File(
        _directory, fileName.substring(fileName.length() - DIGITS_IN_FILENAME) + "." + suffix);
  }

  /**
   * Find the latest snapshot file.
   *
   * <p>Returns null if no snapshot file was found.
   */
   ...
   *
   * <p>Returns null if no appropriate journal file was found.
   */
  public File findInitialJournalFile(long initialTransactionWanted) {
    File[] journals =
        _directory.listFiles(
            new FileFilter() {
   ...
   *
   * <p>Necessary files include the latest snapshot file and any journal files potentially
   * containing transactions after that snapshot version.
   */
