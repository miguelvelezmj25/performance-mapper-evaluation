...
public class GenericSnapshotManager<P> {
  ...
  public GenericSnapshotManager(
      Map snapshotSerializers,
    ...
    if (!snapshotSerializers.containsKey(primarySnapshotSuffix)) {
      throw new IllegalArgumentException(
          "Primary suffix '" + primarySnapshotSuffix + "' does not appear in strategies map");
    }
    ...
    long recoveredVersion =
        latestSnapshot == null ? 0 : PrevaylerDirectory.snapshotVersion(latestSnapshot);
    P recoveredPrevalentSystem =
        latestSnapshot == null ? newPrevalentSystem : readSnapshot(latestSnapshot);
    _recoveredPrevalentSystem =
        new PrevalentSystemGuard<P>(recoveredPrevalentSystem, recoveredVersion, journalSerializer);
  }
  ...
  GenericSnapshotManager(P newPrevalentSystem) {
    ...
    _directory = null;
    _recoveredPrevalentSystem =
        new PrevalentSystemGuard<P>(newPrevalentSystem, 0, new JavaSerializer());
  }
