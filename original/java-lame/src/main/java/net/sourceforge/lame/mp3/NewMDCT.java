        ...
        for(int ch = 0; ch < gfc.channels_out; ch++) {
            for(int gr = 0; gr < gfc.mode_gr; gr++) {
                ...
                for(band = 0; band < 32; band++, mdct_encPos += 18) {
                    ...
                    if(Sink.getDecision(gi.mixed_block_flag != 0 && band < 2)) {
                        type = 0;
                    }
                    if(Sink.getDecision(gfc.amp_filter[band] < 1e-12)) {
                        Arrays.fill(mdct_enc, mdct_encPos + 0,
                                mdct_encPos + 18, 0);
                    }
                    else {
                        if(Sink.getDecision(gfc.amp_filter[band] < 1.0)) {
                            for(int k = 0; k < 18; k++)
                                band1[k][order[band]] *= gfc.amp_filter[band];
                        }
                        if(Sink.getDecision(type == Encoder.SHORT_TYPE)) {
					 ...
					 */
                    if(Sink.getDecision(type != Encoder.SHORT_TYPE && band != 0)) {
                        for(int k = 7; k >= 0; --k) {
                            float bu, bd;
            ...
            wk = w1;
            wkPos = 286;
            if(Sink.getDecision(gfc.mode_gr == 1)) {
                for(int i = 0; i < 18; i++) {
