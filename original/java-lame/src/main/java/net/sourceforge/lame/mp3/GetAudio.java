...
public class GetAudio {
    ...
    public final void close_infile() {
        if(Sink.getDecision(musicin != null)) {
            try {
                musicin.close();
            } catch (IOException e) {
    ...
    private int get_audio_common(final LameGlobalFlags gfp,
		 ...
		 */
        if(Sink.getDecision(count_samples_carefully)) {
            remaining = tmp_num_samples
                    - Math.min(tmp_num_samples, num_samples_read);
            if(Sink.getDecision(remaining < framesize && 0 != tmp_num_samples))
				/*
        ...
        if(Sink.getDecision(is_mpeg_file_format(parse.getInputFormat()))) {
            if(Sink.getDecision(buffer != null)) {
                samples_read = read_samples_mp3(gfp, musicin, buf_tmp16);
            }
            else {
                samples_read = read_samples_mp3(gfp, musicin, buffer16);
            }
            if(Sink.getDecision(samples_read < 0)) {
                return samples_read;
            }
        }
            ...
            if(Sink.getDecision(buffer != null)) { /* output to int buffer )*/
                if(Sink.getDecision(num_channels == 2)) {
                    for(int i = samples_read; --i >= 0; ) {
                        buffer[1][i] = insamp[--p];
                        buffer[0][i] = insamp[--p];
                    }
                }
                else if(Sink.getDecision(num_channels == 1)) {
		 ...
		 */
        if(Sink.getDecision(tmp_num_samples != Integer.MAX_VALUE)) {
    ...
    int read_samples_mp3(final LameGlobalFlags gfp, RandomAccessFile musicin,
		 ...
		 */
        if(Sink.getDecision(out < 0)) {
            Arrays.fill(mpg123pcm[0], (short) 0);
            Arrays.fill(mpg123pcm[1], (short) 0);
            return 0;
        }

        if(Sink.getDecision(gfp.getInNumChannels() != parse.getMp3InputData().stereo)) {
            if(Sink.getDecision(parse.silent < 10)) {
                System.err
        ...
        if(Sink.getDecision(gfp.getInSampleRate() != parse.getMp3InputData().samplerate)) {
            if(Sink.getDecision(parse.silent < 10)) {
                System.err
    ...
    private int unpack_read_samples(final int samples_to_read,
        ...
        int op = samples_read;
        if(Sink.getDecision(!swap_order)) {
            if(Sink.getDecision(bytes_per_sample == 1)) {
                for(int i = samples_read * bytes_per_sample; (i -= bytes_per_sample) >= 0; )
                    sample_buffer[--op] = (bytes[i] & 0xff) << 24;
            }
            if(Sink.getDecision(bytes_per_sample == 2)) {
                for(int i = samples_read * bytes_per_sample; (i -= bytes_per_sample) >= 0; )
                    sample_buffer[--op] = (bytes[i] & 0xff) << 16
                            | (bytes[i + 1] & 0xff) << 24;
            }
            if(Sink.getDecision(bytes_per_sample == 3)) {
                for(int i = samples_read * bytes_per_sample; (i -= bytes_per_sample) >= 0; )
                    sample_buffer[--op] = (bytes[i] & 0xff) << 8
                            | (bytes[i + 1] & 0xff) << 16
                            | (bytes[i + 2] & 0xff) << 24;
            }
            if(Sink.getDecision(bytes_per_sample == 4)) {
                for(int i = samples_read * bytes_per_sample; (i -= bytes_per_sample) >= 0; )
    ...
    private int read_samples_pcm(final RandomAccessFile musicin,
        ...
        try {
            switch (pcmbitwidth) {
                ...
                case 24:
                case 16:
                    if(Sink.getDecision(!parse.in_signed)) {
                    ...
                    }
                {
                    ...
                    if(Sink.getDecision(pcmswapbytes)) {
    ...
    private int parse_wave_header(final LameGlobalFlags gfp,
        ...
        if(Sink.getDecision(Read32BitsHighLow(sf) != WAV_ID_WAVE)) {
            return -1;
        }
        ...
        for(loop_sanity = 0; loop_sanity < 20; ++loop_sanity) {
            ...
            if(Sink.getDecision(type == WAV_ID_FMT)) {
                subSize = Read32BitsLowHigh(sf);
                if(Sink.getDecision(subSize < 16)) {
                    return -1;
                }
                ...
                if(Sink.getDecision((subSize > 9) && (format_tag == WAVE_FORMAT_EXTENSIBLE))) {
            ...
            else if(Sink.getDecision(type == WAV_ID_DATA)) {
                subSize = Read32BitsLowHigh(sf);
                data_length = subSize;
                is_wav = true;
        ...
        if(Sink.getDecision(is_wav)) {
            if(Sink.getDecision(format_tag != WAVE_FORMAT_PCM)) {
                if(Sink.getDecision(parse.silent < 10)) {
            ...
            if(Sink.getDecision(-1 == gfp.getInNumChannels())) {
                if(Sink.getDecision(parse.silent < 10)) {
    ...
    private int aiff_check2(final IFF_AIFF pcm_aiff_data) {
        if(Sink.getDecision(pcm_aiff_data.sampleType != IFF_ID_SSND)) {
            if(Sink.getDecision(parse.silent < 10)) {
        ...
        if(Sink.getDecision(pcm_aiff_data.numChannels != 1 && pcm_aiff_data.numChannels != 2)) {
            if(Sink.getDecision(parse.silent < 10)) {
                System.err
        ...
        if(Sink.getDecision(pcm_aiff_data.blkAlgn.blockSize != 0)) {
            if(Sink.getDecision(parse.silent < 10)) {
                System.err
    ...
    private int parse_aiff_header(final LameGlobalFlags gfp,
        ...
        if(Sink.getDecision((typeID != IFF_ID_AIFF) && (typeID != IFF_ID_AIFC))) {
            return -1;
        }
        ...
        while (chunkSize > 0) {
            ...
            if(Sink.getDecision(type == IFF_ID_COMM)) {
                ...
                ckSize -= 10;
                if(Sink.getDecision(typeID == IFF_ID_AIFC)) {
            ...
            else if(Sink.getDecision(type == IFF_ID_SSND)) {
                seen_ssnd_chunk = 1;
                subSize = Read32BitsHighLow(sf);
                ckSize = make_even_number_of_bytes_in_length(subSize);
                chunkSize -= ckSize;
                ...
                if(Sink.getDecision(seen_comm_chunk > 0)) {
                    try {
                ...
                if(Sink.getDecision(pcm_data_pos >= 0)) {
        ...
        if(Sink.getDecision(dataType == IFF_ID_2CLE)) {
            pcmswapbytes = parse.swapbytes;
        }
        else if(Sink.getDecision(dataType == IFF_ID_2CBE)) {
            pcmswapbytes = !parse.swapbytes;
        }
        else if(Sink.getDecision(dataType == IFF_ID_NONE)) {
            pcmswapbytes = !parse.swapbytes;
        }
        else {
            return -1;
        ...
        if(Sink.getDecision(seen_comm_chunk != 0
			...
			/* make sure the header is sane */
            if(Sink.getDecision(0 != aiff_check2(aiff_info))) {
                return 0;
            }
    ...
    private SoundFileFormat parse_file_header(final LameGlobalFlags gfp,
        ...
        if(Sink.getDecision(type == WAV_ID_RIFF)) {
			/* It's probably a WAV file */
            int ret = parse_wave_header(gfp, sf);
            if(Sink.getDecision(ret > 0)) {
        ...
        else if(Sink.getDecision(type == IFF_ID_FORM)) {
    ...
    private RandomAccessFile OpenSndFile(final LameGlobalFlags gfp,
        ...
        if(Sink.getDecision(is_mpeg_file_format(parse.getInputFormat()))) {
            if(Sink.getDecision(-1 == lame_decode_initfile(musicin, parse.getMp3InputData(), enc))) {
        ...
        else if(Sink.getDecision(parse.getInputFormat() == SoundFileFormat.sf_ogg)) {
            throw new RuntimeException("sorry, vorbis support in LAME is deprecated.");
        }
        else if(Sink.getDecision(parse.getInputFormat() == SoundFileFormat.sf_raw)) {
			/* assume raw PCM */
            if(Sink.getDecision(parse.silent < 10)) {
                System.out.println("Assuming raw pcm input file");
                if(Sink.getDecision(parse.swapbytes)) {
        ...
        if(Sink.getDecision(parse.getInputFormat() == SoundFileFormat.sf_unknown)) {
            throw new RuntimeException("Unknown sound format!");
        }

        if(Sink.getDecision(gfp.num_samples == -1)) {

            double flen = new File(inPath).length();
			/* try to figure out num_samples */
            if(Sink.getDecision(flen >= 0)) {
				/* try file size, assume 2 bytes per sample */
                if(Sink.getDecision(is_mpeg_file_format(parse.getInputFormat()))) {
                    if(Sink.getDecision(parse.getMp3InputData().bitrate > 0)) {
    ...
    private boolean is_syncword_mp123(final byte[] headerptr) {
        ...
        if(Sink.getDecision((headerptr[p + 0] & 0xFF) != 0xFF)) {
			/* first 8 bits must be '1' */
            return false;
        }
        if(Sink.getDecision((headerptr[p + 1] & 0xE0) != 0xE0)) {
			/* next 3 bits are also */
            return false;
        }
        if(Sink.getDecision((headerptr[p + 1] & 0x18) == 0x08)) {
			/* no MPEG-1, -2 or -2.5 */
            return false;
        }
        switch (headerptr[p + 1] & 0x06) {
            ...
            case 0x02:
			/* Layer3 */
                if(Sink.getDecision(parse.getInputFormat() != SoundFileFormat.sf_mp3
            ...
            case 0x04:
			/* Layer2 */
                if(Sink.getDecision(parse.getInputFormat() != SoundFileFormat.sf_mp2
                        && parse.getInputFormat() != SoundFileFormat.sf_mp123)) {
                    return false;
                }
            ...
            case 0x06:
			/* Layer1 */
                if(Sink.getDecision(parse.getInputFormat() != SoundFileFormat.sf_mp1
                        && parse.getInputFormat() != SoundFileFormat.sf_mp123)) {
                    return false;
                }
        ...
        if(Sink.getDecision((headerptr[p + 1] & 0x06) == 0x00)) {
			/* no Layer I, II and III */
            return false;
        }
        if(Sink.getDecision((headerptr[p + 2] & 0xF0) == 0xF0)) {
			/* bad bitrate */
            return false;
        }
        if(Sink.getDecision((headerptr[p + 2] & 0x0C) == 0x0C)) {
			/* no sample frequency with (32,44.1,48)/(1,2,4) */
            return false;
        }
        if(Sink.getDecision((headerptr[p + 1] & 0x18) == 0x18
                && (headerptr[p + 1] & 0x06) == 0x04
                && (abl2[(headerptr[p + 2] & 0xff) >> 4] & (1 << ((headerptr[p + 3] & 0xff) >> 6))) != 0)) {
            return false;
        }
        if(Sink.getDecision((headerptr[p + 3] & 3) == 2)) {
    ...
    private int lame_decode_initfile(final RandomAccessFile fd,
        ...
        if(Sink.getDecision(buf[0] == 'I' && buf[1] == 'D' && buf[2] == '3')) {
            if(Sink.getDecision(parse.silent < 10)) {
                System.out
                        .println("ID3v2 found. "
        ...
        if(Sink.getDecision(check_aid(buf))) {
            try {
        ...
        if(Sink.getDecision((buf[2] & 0xf0) == 0)) {
            if(Sink.getDecision(parse.silent < 10)) {
        ...
        if(Sink.getDecision(mp3data.bitrate == 0 && !freeformat)) {
            if(Sink.getDecision(parse.silent < 10)) {
        ...
        if(Sink.getDecision(mp3data.getTotalFrames() > 0)) {
    ...
    private double convertFromIeeeExtended(final byte[] bytes) {
		 ...
		 */
        if(Sink.getDecision(expon == 0 && hiMant == 0 && loMant == 0)) {
            f = 0;
        }
        else {
			/*
            ...
            if(Sink.getDecision(expon == 0x7FFF)) { /* Infinity or NaN )*/
        ...
        if(Sink.getDecision((bytes[0] & 0x80) != 0)) {
            return -f;
        }
        else {
            return f;
