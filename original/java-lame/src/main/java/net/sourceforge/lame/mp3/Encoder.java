...
public class Encoder {
    ...
    private void adjust_ATH(final LameInternalFlags gfc) {
        ...
        if(Sink.getDecision(gfc.ATH.useAdjust == 0)) {
            gfc.ATH.adjust = 1.0f;
      /* no adjustment */
            return;
        }
        ...
        if(Sink.getDecision(gfc.channels_out == 2)) {
        ...
        if(Sink.getDecision(gfc.mode_gr == 2)) {
        ...
        if(Sink.getDecision(max_pow > 0.03125)) { /* ((1 - 0.000625)/ 31.98) from curve below )*/
            if(Sink.getDecision(gfc.ATH.adjust >= 1.0)) {
                gfc.ATH.adjust = 1.0f;
            }
            else {
                ...
                if(Sink.getDecision(gfc.ATH.adjust < gfc.ATH.adjustLimit)) {
            ...
            if(Sink.getDecision(gfc.ATH.adjust >= adj_lim_new)) { /* descend gradually )*/
                gfc.ATH.adjust *= adj_lim_new * 0.075 + 0.925;
                if(Sink.getDecision(gfc.ATH.adjust < adj_lim_new)) { /* stop descent )*/
                    gfc.ATH.adjust = adj_lim_new;
                }
            }
            else { /* ascend */
                if(Sink.getDecision(gfc.ATH.adjustLimit >= adj_lim_new)) {
                    gfc.ATH.adjust = adj_lim_new;
                }
                else {
					/* preceding frame has lower ATH adjust; */
    ...
    private void updateStats(final LameInternalFlags gfc) {
        ...
        for(gr = 0; gr < gfc.mode_gr; ++gr) {
            for(ch = 0; ch < gfc.channels_out; ++ch) {
                ...
                if(Sink.getDecision(gfc.l3_side.tt[gr][ch].mixed_block_flag != 0)) {
                    bt = 4;
                }
    ...
    private void lame_encode_frame_init(final LameGlobalFlags gfp,
        ...
        if(Sink.getDecision(gfc.lame_encode_frame_init == 0)) {
            ...
            for(i = 0, j = 0; i < 286 + 576 * (1 + gfc.mode_gr); ++i) {
                if(Sink.getDecision(i < 576 * gfc.mode_gr)) {
                    primebuff0[i] = 0;
                    if(Sink.getDecision(gfc.channels_out == 2)) {
                        primebuff1[i] = 0;
                    }
    ...
    public final int lame_encode_mp3_frame(final LameGlobalFlags gfp,
         ...
         */
        gfc.padding = 0;
        if(Sink.getDecision((gfc.slot_lag -= gfc.frac_SpF) < 0)) {
        ...
        if(Sink.getDecision(gfc.psymodel != 0)) {
			/*
            ...
            for(gr = 0; gr < gfc.mode_gr; gr++) {
                ...
                if(Sink.getDecision(gfp.getVBR() == VbrMode.vbr_mtrh || gfp.getVBR() == VbrMode.vbr_mt)) {
                ...
                if(Sink.getDecision(gfp.getMode() == MPEGMode.JOINT_STEREO)) {
                    ms_ener_ratio[gr] = tot_ener[gr][2] + tot_ener[gr][3];
                    if(Sink.getDecision(ms_ener_ratio[gr] > 0)) {
        ...
        if(Sink.getDecision(gfp.force_ms)) {
            gfc.mode_ext = MPG_MD_MS_LR;
        }
        else if(Sink.getDecision(gfp.getMode() == MPEGMode.JOINT_STEREO)) {
			/*
            ...
            if(Sink.getDecision(sum_pe_MS <= 1.00 * sum_pe_LR)) {

                final GrInfo gi0[] = gfc.l3_side.tt[0];
                final GrInfo gi1[] = gfc.l3_side.tt[gfc.mode_gr - 1];

                if(Sink.getDecision(gi0[0].block_type == gi0[1].block_type
        ...
        if(Sink.getDecision(gfc.mode_ext == MPG_MD_MS_LR)) {
        ...
        if(Sink.getDecision(gfp.analysis && gfc.pinfo != null)) {
            for(gr = 0; gr < gfc.mode_gr; gr++) {
                for(ch = 0; ch < gfc.channels_out; ch++) {
        ...
        if(Sink.getDecision(gfp.getVBR() == VbrMode.vbr_off || gfp.getVBR() == VbrMode.vbr_abr)) {
        ...
        if(Sink.getDecision(gfp.bWriteVbrTag)) {
            vbr.addVbrFrame(gfp);
        }
