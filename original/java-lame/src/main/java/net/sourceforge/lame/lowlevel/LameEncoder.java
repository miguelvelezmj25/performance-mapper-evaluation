...
public class LameEncoder {
    ...
    private void readParams(AudioFormat sourceFormat, Map<String, Object> props) {
        if(Sink.getDecision(props != null)) {
            readProps(props);
        }
    }

    public void initParams(AudioFormat sourceFormat) {
    ...
    private int doEncodeBuffer(final byte[] pcm, final int pcmOffset,
        ...
        if(Sink.getDecision(lame.getFlags().getInNumChannels() == 2)) {
            for(int i = samplesRead; --i >= 0; ) {
                buffer[1][i] = sampleBuffer[--sampleBufferPos];
                buffer[0][i] = sampleBuffer[--sampleBufferPos];
            }
        }
        else if(Sink.getDecision(lame.getFlags().getInNumChannels() == 1)) {
    ...
    public final int getEffectiveQuality() {
        if(Sink.getDecision(effQuality >= Lame.QUALITY_LOWEST)) {
            return Lame.QUALITY_LOWEST;
        }
        else if(Sink.getDecision(effQuality >= Lame.QUALITY_LOW)) {
            return Lame.QUALITY_LOW;
        }
        else if(Sink.getDecision(effQuality >= Lame.QUALITY_MIDDLE)) {
            return Lame.QUALITY_MIDDLE;
        }
        else if(Sink.getDecision(effQuality >= Lame.QUALITY_HIGH)) {
    ...
    public final AudioFormat.Encoding getEffectiveEncoding() {
        if(Sink.getDecision(effEncoding == MPEG_VERSION_2)) {
            if(Sink.getDecision(getEffectiveSampleRate() < 16000)) {
                return MPEG2DOT5L3;
            }
            return MPEG2L3;
        }
        else if(Sink.getDecision(effEncoding == MPEG_VERSION_2DOT5)) {
            return MPEG2DOT5L3;
        }
        // default
        return MPEG1L3;
