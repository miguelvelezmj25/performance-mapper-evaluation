    ...
    public Result convert(File srcImage, Arguments args) {
        try {
            ...
            log.append(getConverterName()).append(": ").append(targetImageFileName).append(" ")
                    .append(imageData.getImage().getWidth()).append("x").append(imageData.getImage().getHeight()).append(" (").append(args.scale).append((args.scaleMode == EScaleMode.FACTOR || args.scaleIsHeightdp) ? "x" : "dp").append(")\n");
            ...
            if(args.scale >= 0 || args.platform != null || args.scaleIsHeightdp || args.scaleMode != null || args.skipUpscaling) {
