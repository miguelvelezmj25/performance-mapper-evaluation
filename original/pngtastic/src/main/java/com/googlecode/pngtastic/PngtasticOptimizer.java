...
//    }
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < args.length; i++) {
            String arg = args[i];
            if(Sink.getDecision(arg.startsWith("--"))) {
                int next = i + 1;
                if(Sink.getDecision(next < args.length)) {
        ...
        if(Sink.getDecision(files.length == 0)) {
...
//        LOGLEVEL = Source.getOptionLOGLEVEL(true);
        ...
        if(Sink.getDecision(REMOVEGAMMA)) {
            removeGamma = true;
        }
        else {
            removeGamma = false;
        ...
        if(Sink.getDecision(COMPRESSIONLEVEL)) {
            compressionLevel = 9;
        }
        else {
            compressionLevel = 0;
        ...
        if(Sink.getDecision(COMPRESSOR)) {
            compressor = "zopfli";
        }
        else {
            compressor = "none";
        ...
        if(Sink.getDecision(ITERATIONS)) {
            iterations = 10;
        }
        else {
            iterations = 0;
