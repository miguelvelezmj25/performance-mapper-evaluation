...
import counter.com.googlecode.pngtastic.core.PngFilterType;
...
public class PngtasticFilterHandler implements PngFilterHandler {
  ...
  /** {@inheritDoc} */
  @Override
  public void applyAdaptiveFiltering(
   ...
   * {@inheritDoc}
   *
   * <p>The bytes are named as follows (x = current, a = previous, b = above, c = previous and
   * above)
   *
   * <pre>
   * c b
      ...
      case AVERAGE:
        {
          ...
          for (int x = 1, a = previous; x < line.length; x++, a++) {
            line[x] =
                (byte)
                    (original[x]
                        - ((0xFF & original[(a < 0) ? 0 : a]) + (0xFF & previousLine[x])) / 2);
          }
          break;
        }
      case PAETH:
      ...
      case AVERAGE:
        {
          ...
          for (int x = 1, a = previous; x < line.length; x++, a++) {
            line[x] =
                (byte)
                    (line[x] + ((0xFF & ((a < 0) ? 0 : line[a])) + (0xFF & previousLine[x])) / 2);
          }
          break;
        }
      case PAETH:
