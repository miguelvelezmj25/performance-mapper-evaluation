...
public class PngtasticInterlaceHandler implements PngInterlaceHandler {
   ...
   * {@inheritDoc}
   *
   * <p>Throws a runtime exception.
   *
   * <p>NOTE: This is left unimplemented currently. Interlacing should make most images larger in
   * filesize, so pngtastic currently deinterlaces all images passed through it. There may be rare
   * exceptions that actually benefit from interlacing, so there may come a time to revisit this.
   */
  @Override
  ...
  public List<byte[]> deInterlace(
    ...
    final byte[][] rows =
        new byte[height][Double.valueOf(Math.ceil(width * sampleBitCount / 8D)).intValue() + 1];

    int subImageOffset = 0;
    for (int pass = 0; pass < 7; pass++) {
      final int subImageRows =
          (((height - interlaceRowOffset[pass]) + (interlaceRowFrequency[pass] - 1))
              / interlaceRowFrequency[pass]);
      final int subImageColumns =
          (((width - interlaceColumnOffset[pass]) + (interlaceColumnFrequency[pass] - 1))
              / interlaceColumnFrequency[pass]);
      final int rowLength =
          Double.valueOf(Math.ceil(subImageColumns * sampleBitCount / 8D)).intValue() + 1;
