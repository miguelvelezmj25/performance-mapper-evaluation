...
public class PersistentJournal implements Journal {
  ...
  private void initializeNextTransaction(long initialTransactionWanted, long nextTransaction)
    ...
    _nextTransaction =
        initialTransactionWanted > nextTransaction ? initialTransactionWanted : nextTransaction;
  }
  ...
  private void abort(Exception exception, File journal, String action, Guided guide) {
    guide.abortTurn(
        "All transaction processing is now aborted. An IOException was thrown while "
            + action
            + " a .journal file.",
        exception);
  }
