...
public class PrevaylerImpl<P> implements Prevayler<P> {
   ...
   *
   * @param snapshotManager The SnapshotManager that will be used for reading and writing snapshot
   *     files.
   * @param transactionPublisher The TransactionPublisher that will be used for publishing
   *     transactions executed with this PrevaylerImpl.
   * @param journalSerializer
   */
  public PrevaylerImpl(
      GenericSnapshotManager<P> snapshotManager,
  ...
  public void execute(Transaction<? super P> transaction) {
    publish(
        new TransactionCapsule<P>(
            transaction,
            _journalSerializer,
            _transactionDeepCopyMode)); // TODO Optimization: The Censor can use the actual given
                                        // transaction if it is Immutable instead of deserializing a
                                        // new one from the byte array, even if
                                        // "_transactionDeepCopyMode" is "true"
  }
  ...
  public <R> R execute(TransactionWithQuery<? super P, R> transactionWithQuery) throws Exception {
    TransactionWithQueryCapsule<? super P, R> capsule =
        new TransactionWithQueryCapsule<P, R>(
            transactionWithQuery, _journalSerializer, _transactionDeepCopyMode);
    publish(capsule);
