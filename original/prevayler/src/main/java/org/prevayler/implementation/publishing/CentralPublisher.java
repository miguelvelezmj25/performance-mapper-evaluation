...
public class CentralPublisher extends AbstractPublisher {
  ...
  public CentralPublisher(Clock clock, Journal journal) {
    ...
    _pausableClock =
        (PausableClock) _clock; // This is just to avoid casting the inherited _clock every time.
  ...
  public void publish(Capsule capsule) {
    synchronized (
        _pendingPublicationsMonitor) { // Blocks all new subscriptions until the publication is
                                       // over.
    ...
    try {
      publishWithoutWorryingAboutNewSubscriptions(
          capsule); // Suggestions for a better method name are welcome.  :)
    } finally {
  ...
  private TransactionGuide guideFor(Capsule capsule) {
    synchronized (_nextTurnMonitor) {
      TransactionTimestamp timestamp =
          new TransactionTimestamp(capsule, _nextTransaction, _pausableClock.realTime());
