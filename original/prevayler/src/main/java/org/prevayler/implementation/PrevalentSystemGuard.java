...
public class PrevalentSystemGuard<P> implements TransactionSubscriber {
  ...
  private P
      _prevalentSystem; // All access to field is synchronized on "this", and all access to object
  ...
  public void subscribeTo(TransactionPublisher publisher)
    ...
    synchronized (this) {
      _ignoreRuntimeExceptions =
          true; // During pending transaction recovery (rolling forward), RuntimeExceptions are
                // ignored because they were already thrown and handled during the first transaction
                // execution.
    ...
    synchronized (this) {
      ...
      try {
        ...
        if (!_ignoreRuntimeExceptions)
          throw rx; // TODO Guarantee that transactions received from pending transaction recovery
                    // don't ever throw RuntimeExceptions. Maybe use a wrapper for that.
      } catch (Error error) {
  ...
  public PrevalentSystemGuard<P> deepCopy(long systemVersion, Serializer snapshotSerializer)
      throws Exception {
    synchronized (this) {
      ...
      if (_systemVersion > systemVersion) {
        throw new IllegalStateException(
            "Already at " + _systemVersion + "; can't go back to " + systemVersion);
      }

      synchronized (_prevalentSystem) {
        return new PrevalentSystemGuard<P>(
            (P) DeepCopier.deepCopyParallel(_prevalentSystem, snapshotSerializer),
            _systemVersion,
            _journalSerializer);
      }
    }
