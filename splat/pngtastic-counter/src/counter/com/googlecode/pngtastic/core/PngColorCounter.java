...
public class PngColorCounter extends PngProcessor {
    ...
    private final DoubleSupplier distThreshold;
    private final DoubleSupplier freqThreshold;
    private final IntSupplier minAlpha;
...
//    }
    ...
    public PngColorCounter(Supplier<String> logLevel, DoubleSupplier distThreshold, DoubleSupplier freqThreshold, IntSupplier minAlpha, IntSupplier timeout) {
        super(logLevel);
    ...
    private List<PngPixel> getColors(PngImage original, List<byte[]> rows, long start) throws IOException {
        ...
        int y = 0;
        for(byte[] row : rows) {
            if(timeout.getAsInt() > 0 && (System.currentTimeMillis() - start > timeout.getAsInt())) {
            ...
            for(int x = 0; x < sampleCount; x++) {
                switch (imageType) {
                    ...
                    case TRUECOLOR_ALPHA: {
                        ...
                        if(pixel.getAlpha() > minAlpha.getAsInt()) {
                            final Integer count = colors.get(pixel);
                            colors.put(pixel, (count == null) ? 1 : (count + 1));
                        }
                        break;
        ...
        if(freqThreshold.getAsDouble() > 0) {
            final int minFreq = (int) (original.getWidth() * original.getHeight() * freqThreshold.getAsDouble());
        ...
        for(PngPixel pa : colors) {
            ...
            if(!pa.isDuplicate()) {
                for(Iterator<PngPixel> it = copy.iterator(); it.hasNext(); ) {
                    ...
                    else if(pa != pb && pa.rgbaDistance(pb, bits) < distThreshold.getAsDouble()) {
