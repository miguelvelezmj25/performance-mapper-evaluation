package optimizer.com.googlecode.pngtastic.core;

import optimizer.com.googlecode.pngtastic.core.processing.PngByteArrayOutputStream;
import optimizer.com.googlecode.pngtastic.core.processing.ZopfliCompressionHandler;
...
//    }
    ...
    public PngOptimizer(Supplier<String> logLevel) {
        super(logLevel);
    }
    ...
    /** */
    public void optimize(PngImage image, String outputFileName, BooleanSupplier removeGamma, IntSupplier compressionLevel)
            throws IOException {
    ...
    /** */
    public PngImage optimize(PngImage image, BooleanSupplier removeGamma, IntSupplier compressionLevel) throws IOException {
...
//		Map<PngPixel, Integer> colors = getColors(image, originalScanlines, 32);
        ...
        for(Entry<PngFilterType, List<byte[]>> entry : filteredScanlines.entrySet()) {
            final byte[] imageResult = pngCompressionHandler.deflate(serialize(entry.getValue()), compressionLevel, false);
        ...
        final byte[] adaptiveImageData = pngCompressionHandler.deflate(inflatedImageData, compressionLevel, false);
    ...
    public void setCompressor(Supplier<String> compressor, IntSupplier iterations) {
        if("zopfli".equals(compressor.get())) {
            if(iterations != null) {
