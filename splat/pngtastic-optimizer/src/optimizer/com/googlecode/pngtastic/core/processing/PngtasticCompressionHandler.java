package optimizer.com.googlecode.pngtastic.core.processing;

import optimizer.com.googlecode.pngtastic.core.Logger;
...
public class PngtasticCompressionHandler implements PngCompressionHandler {
     ...
     */
    @Override
    public byte[] deflate(PngByteArrayOutputStream inflatedImageData, IntSupplier compressionLevel, boolean concurrent) throws IOException {
...
//    }
    ...
    /* */
    private List<byte[]> deflateImageDataSerially(PngByteArrayOutputStream inflatedImageData, IntSupplier compressionLevel, Integer compressionStrategy) {
    ...
    /* */
    private byte[] deflateImageData(PngByteArrayOutputStream inflatedImageData, int strategy, IntSupplier compressionLevel) throws IOException {
        byte[] result = null;
        int bestCompression = Deflater.BEST_COMPRESSION;

        if(compressionLevel == null || compressionLevel.getAsInt() > Deflater.BEST_COMPRESSION || compressionLevel.getAsInt() < Deflater.NO_COMPRESSION) {
            ...
            result = deflate(inflatedImageData, strategy, compressionLevel.getAsInt()).toByteArray();
            bestCompression = compressionLevel.getAsInt();
        }
