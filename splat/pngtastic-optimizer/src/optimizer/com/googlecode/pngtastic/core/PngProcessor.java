...
import optimizer.com.googlecode.pngtastic.core.processing.*;
...
public abstract class PngProcessor {
    ...
    protected PngProcessor(Supplier<String> logLevel) {
    ...
    protected PngChunk processHeadChunks(PngImage result, BooleanSupplier removeGamma, Iterator<PngChunk> itChunks) throws IOException {
        PngChunk chunk = null;
        while (itChunks.hasNext()) {
            ...
            if(result != null && chunk.isRequired()) {
                if(removeGamma.getAsBoolean() && PngChunk.IMAGE_GAMA.equalsIgnoreCase(chunk.getTypeString())) {
                    continue;
                }
