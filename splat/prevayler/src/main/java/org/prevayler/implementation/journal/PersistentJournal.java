...
public class PersistentJournal implements Journal {
    ...
    private final LongSupplier _journalSizeThresholdInBytes;
    private final LongSupplier _journalAgeThresholdInMillis;
    private final BooleanSupplier _journalDiskSync;
     ...
     * @param monitor
     */
    public PersistentJournal(PrevaylerDirectory directory, LongSupplier journalSizeThresholdInBytes, LongSupplier journalAgeThresholdInMillis,
                             BooleanSupplier journalDiskSync, String journalSuffix, Supplier<Monitor> monitor) throws IOException {
    ...
    private boolean isOutputJournalTooOld() {
        return _journalAgeThresholdInMillis.getAsLong() != 0
                && _journalAgeTimer.millisEllapsed() >= _journalAgeThresholdInMillis.getAsLong();
    }


    private boolean isOutputJournalTooBig() {
        return _journalSizeThresholdInBytes.getAsLong() != 0
                && _outputJournal.file().length() >= _journalSizeThresholdInBytes.getAsLong();
    }
