...
//    }
    ...
    public CompressedOutputStream(OutputStream os, IntSupplier blockSize, BooleanSupplier checksum, ExecutorService pool, int jobs, String codec, Supplier<String> transform) {
//        if(os == null) {
//            throw new NullPointerException("Invalid null output stream parameter");
//        }
//
...
//        }
        ...
        final int bufferSize = (blockSize.getAsInt() <= 65536) ? blockSize.getAsInt() : 65536;
        this.obs = new DefaultOutputBitStream(os, bufferSize);
        this.entropyType = new EntropyCodecFactory().getType(codec);
        this.transformType = new ByteFunctionFactory().getType(transform.get());
        this.blockSize = blockSize;
        this.hasher = (checksum.getAsBoolean() == true) ? new XXHash32(BITSTREAM_TYPE) : null;
        this.jobs = (jobs == 0) ? 1 : jobs;
        this.pool = pool;
        this.sa = new SliceByteArray(new byte[this.blockSize.getAsInt() * this.jobs], 0);
        this.buffers = new SliceByteArray[2 * this.jobs];
        this.closed = new AtomicBoolean(false);
        this.initialized = new AtomicBoolean(false);
...
//        this.ctx = ctx;
    ...
    protected void writeHeader() throws IOException {
        ...
        if(this.obs.writeBits(this.blockSize.getAsInt() >>> 4, 26) != 26) {
    ...
    private void processBlock() throws IOException {
        ...
        try {
            ...
            for(int jobId = 0; jobId < this.jobs; jobId++) {
                final int sz = (this.sa.index + this.blockSize.getAsInt() > dataLength) ?
                        dataLength - this.sa.index : this.blockSize.getAsInt();
...
//                        blockListeners, new HashMap<>(this.ctx));
                Callable<Status> task = new EncodingTask(this.buffers[2 * jobId],
                        ...
                        this.entropyType, firstBlockId + jobId + 1,
                        this.obs, this.hasher, this.blockId,
                        blockListeners, this.blockSize.getAsInt());
