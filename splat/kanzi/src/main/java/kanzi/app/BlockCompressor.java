...
public class BlockCompressor implements Runnable, Callable<Integer> {
    ...
    private final IntSupplier verbosity;
    private final BooleanSupplier overwrite;
    private final BooleanSupplier checksum;
    private final String inputName;
    private final String outputName;
    private final String codec;
    private final Supplier<String> transform;
    private final IntSupplier blockSize;
    private final IntSupplier level; // command line compression level
...
//    }
    ...
    public BlockCompressor(IntSupplier verbose, BooleanSupplier force, IntSupplier blockSize, IntSupplier level, Supplier<String> entropy, Supplier<String> transform,
                           BooleanSupplier checksum, int jobs, Map<String, Object> map, ExecutorService threadPool) {
        this.level = level;
        this.verbosity = verbose;
        BooleanSupplier bForce = force;
        this.overwrite = (bForce == null) ? () -> false : bForce;
        this.inputName = (String) map.remove("inputName");
        this.outputName = (String) map.remove("outputName");
        String strTransf;
        ...
        if(this.level.getAsInt() >= 0) {
            String tranformAndCodec = getTransformAndCodec(this.level.getAsInt());
            String[] tokens = tranformAndCodec.split("&");
            strTransf = tokens[0];
            strCodec = tokens[1];
            transform.get();
            ...
            strTransf = transform.get();
            strCodec = entropy.get();
        }

        this.codec = (strCodec == null) ? "ANS0" : strCodec;
        IntSupplier iBlockSize = blockSize;
        this.blockSize = (iBlockSize == null) ? () -> DEFAULT_BLOCK_SIZE : iBlockSize;

        // Extract transform names. Curate input (EG. NONE+NONE+xxxx => xxxx)
        ByteFunctionFactory bff = new ByteFunctionFactory();
        this.transform = (strTransf == null) ? () -> "BWT+RANK+ZRLT" : bff.getName(bff.getType(strTransf));
        BooleanSupplier bChecksum = checksum;
        this.checksum = (bChecksum == null) ? () -> false : bChecksum;
        this.jobs = jobs;
        this.pool = (this.jobs < 2) ? null :
                ((threadPool == null) ? Executors.newCachedThreadPool() : threadPool);
        ...
        if(this.verbosity.getAsInt() > 2) {
            this.addListener(new InfoPrinter(this.verbosity.getAsInt(), InfoPrinter.Type.ENCODING, System.out));
        }

        if((this.verbosity.getAsInt() > 0) && (map.size() > 0)) {
            for(String k : map.keySet())
                printOut("Ignoring invalid option [" + k + "]", this.verbosity.getAsInt() > 0);
        }
    }
    ...
    @Override
    public Integer call() {
        boolean printFlag = this.verbosity.getAsInt() > 2;
        printOut("Kanzi 1.2 (C) 2017,  Frederic Langlet", this.verbosity.getAsInt() >= 1);
        ...
        if(this.level.getAsInt() < 0) {
            String etransform = ("NONE".equals(this.transform.get())) ? "no" : this.transform.get();
        ...
        try {
            if(this.outputName.equalsIgnoreCase("NONE")) {
                ...
                if(output.exists()) {
                    ...
                    if(this.overwrite.getAsBoolean() == false) {
...
//                this.cos = new CompressedOutputStream(os, ctx);
        ...
        // Encode
        printFlag = this.verbosity.getAsInt() > 1;
        ...
        printOut("", this.verbosity.getAsInt() >= 1);
        ...
        printOut("Encoding: " + read + " => " + this.cos.getWritten() +
                " bytes in " + delta + " ms", this.verbosity.getAsInt() == 1);
