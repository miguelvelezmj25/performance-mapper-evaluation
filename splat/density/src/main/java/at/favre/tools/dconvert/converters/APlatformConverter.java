    ...
    public Result convert(File srcImage, Arguments args) {
        try {
            ...
            log.append(getConverterName()).append(": ").append(targetImageFileName).append(" ")
                    .append(imageData.getImage().getWidth()).append("x").append(imageData.getImage().getHeight()).append(" (").append(args.scale).append(args.scaleMode.get() == EScaleMode.FACTOR ? "x" : "dp").append(")\n");
            ...
            for (Map.Entry<T, Dimension> entry : densityMap.entrySet()) {
                ...
                if ((dstFolder.isDirectory() && dstFolder.exists()) || args.dryRun.getAsBoolean()) {
