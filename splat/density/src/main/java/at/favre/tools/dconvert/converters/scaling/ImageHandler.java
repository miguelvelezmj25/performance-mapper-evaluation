...
public class ImageHandler {
    ...
    public List<File> saveToFile(File targetFile, LoadedImage imageData, Dimension targetDimension, boolean isNinePatch) throws Exception {
        ...
        for (ImageType.ECompression compression : compressionList) {
            ...
            if (imageFile.exists() && args.skipExistingFiles.getAsBoolean()) {
                break;
            }
    ...
    private void compressJpeg(BufferedImage bufferedImage, CompoundDirectory exif, Supplier<Float> quality, File targetFile) throws IOException {
        ImageWriter jpgWriter = ImageIO.getImageWritersByFormatName("jpg").next();
        ImageWriteParam jpgWriteParam = jpgWriter.getDefaultWriteParam();
        jpgWriteParam.setCompressionMode(ImageWriteParam.MODE_EXPLICIT);
        jpgWriteParam.setCompressionQuality(quality.get());
    ...
    private Supplier<EScalingAlgorithm> getScalingAlgorithm(EScalingAlgorithm.Type type) {
    ...
    private List<ScaleAlgorithm> getScaleAlgorithm(Supplier<EScalingAlgorithm> algorithm, EScalingAlgorithm.Type type) {
        if (TEST_MODE) {
            ...
            return Collections.singletonList(algorithm.get().getImplementation());
        }
    }
    ...
    private BufferedImage scale(ScaleAlgorithm scaleAlgorithm, BufferedImage imageToScale, int dWidth, int dHeight, ImageType.ECompression compression, Color background) {
        ...
        if (args.enableAntiAliasing.getAsBoolean()) {
